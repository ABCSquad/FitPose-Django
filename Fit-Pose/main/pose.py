...
import main.custom_drawing_utils
import main.custom_pose
...
def main_pose(cap, exercise_id):

    
    reps = {}
    initialize_reps(reps)

    mp_drawing = main.custom_drawing_utils   #Using our own custom version of the drawing functions file
    mp_pose = main.custom_pose

    full_keypoints = False
    upper = False
    if int(exercise_id) == 4:                      #Requires full_keypoints to be True
      exercise_name = "bicep_curl"
                                              #Requires full keypoints to be False
    ...
    with mp_pose.Pose(
      ...
      while True:
        ...
        # main.Display reps at down left corner
